...
export const Route = createFileRoute("/faqs")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Complete guidelines, admission protocols, and FAQs for patients at Shree Kalyan Hospital." },
    ],
  }),
});
...
function AnimatedFAQItem({ q, a, num }: { q: string; a: string; num: string }) {
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      gsap.to(wrapperRef.current, { height: contentRef.current.scrollHeight, duration: 0.6, ease: "expo.inOut" });
      gsap.to(contentRef.current, { opacity: 1, y: 0, duration: 0.5, delay: 0.15, ease: "power3.out" });
    } else {
  ...
  return (
    <div className="group border-b border-navy/10 relative overflow-hidden">
      ...
      >
        ...
        <span className={`font-display text-2xl md:text-4xl leading-tight transition-colors duration-500 max-w-3xl ${isOpen ? "text-magenta italic" : "text-navy-deep group-hover:text-magenta"}`}>
          {q}
        </span>
        ...
        <div className="absolute right-0 top-10 md:top-14 size-8 border border-navy/10 rounded-full flex items-center justify-center group-hover:border-magenta transition-colors duration-500">
          <div className="relative size-3 flex items-center justify-center">
            ...
            <span className={`absolute w-full h-[1.5px] bg-navy-deep group-hover:bg-magenta transition-all duration-500 ${isOpen ? "rotate-0" : "rotate-90"}`} />
          </div>
        </div>
      </button>
      ...
      <div ref={wrapperRef} className="h-0 overflow-hidden relative z-10">
        <div ref={contentRef} className="pb-12 pl-0 md:pl-20 opacity-0 -translate-y-4 max-w-3xl">
          <p className="text-lg md:text-xl text-ink/65 font-light leading-relaxed">
            {a}
          </p>
        </div>
      </div>
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(".hq-word", 
        ...
        { yPercent: 0, skewY: 0, opacity: 1, duration: 1.5, stagger: 0.1, ease: "power4.out", delay: 0.2 }
      );
      gsap.fromTo(".hq-line", 
        { x: -50, opacity: 0 }, 
      ...
      // Image sweep in
      gsap.fromTo(".hq-hero-img", 
        { clipPath: "polygon(0 100%, 100% 100%, 100% 100%, 0 100%)", scale: 1.1 },
        { clipPath: "polygon(0 0%, 100% 0%, 100% 100%, 0 100%)", scale: 1, duration: 2, ease: "expo.inOut", delay: 0.1 }
      );
      
      // Image Parallax scroll
      gsap.to(".hq-hero-img-inner", {
        yPercent: 20, ease: "none",
        scrollTrigger: { trigger: ".hq-hero-section", start: "top top", end: "bottom top", scrub: true }
      });
      ...
      gsap.utils.toArray<HTMLElement>(".cat-heading").forEach(heading => {
        gsap.fromTo(heading, 
          { x: -40, opacity: 0 },
          { x: 0, opacity: 1, duration: 1.2, ease: "expo.out", scrollTrigger: { trigger: heading, start: "top 85%", once: true } }
        );
      });
      ...
      gsap.utils.toArray<HTMLElement>(".faq-item-anim").forEach(item => {
        gsap.fromTo(item,
          ...
          { opacity: 1, y: 0, rotateX: 0, duration: 1, ease: "power3.out", scrollTrigger: { trigger: item, start: "top 92%", once: true } }
        );
      });
      ...
      gsap.fromTo(".journey-card",
        { y: 80, opacity: 0 },
        { y: 0, opacity: 1, duration: 1.2, stagger: 0.2, ease: "expo.out", scrollTrigger: { trigger: ".journey-track", start: "top 80%", once: true } }
      );
    ...
    <div ref={pageRef} className="bg-paper text-ink font-sans overflow-x-hidden selection:bg-magenta selection:text-white">
        ...
        <div className="grid lg:grid-cols-12 gap-12 lg:gap-24 items-end relative z-10">
          ...
          <div className="lg:col-span-6 pb-10">
            ...
            <h1 className="font-display leading-[0.88] tracking-tighter text-navy-deep mb-10">
              {["Guidance", "for your", "journey."].map((w, i) => (
                <div key={i} className="overflow-hidden">
                  <span className={`hq-word block ${i===2 ? 'italic font-light text-magenta' : ''}`} style={{ fontSize: "clamp(3.5rem, 8vw, 7rem)" }}>
                    {w}
                  </span>
                </div>
              ))}
            ...
            <p className="hq-line text-lg md:text-xl text-ink/60 font-light leading-relaxed max-w-lg">
              Absolute clarity is the first step toward healing. Find comprehensive details regarding our clinical protocols, admission processes, and global insurance networks.
            </p>
          </div>
        ...
        <div className="max-w-[1600px] mx-auto px-6 md:px-12 lg:px-24">
          <div className="mb-16 flex flex-col md:flex-row md:items-end justify-between gap-8">
            ...
            <p className="max-w-sm text-paper/60 font-light leading-relaxed">
              To ensure a seamless experience, we request you to follow these essential preparatory steps prior to your scheduled consultation or admission.
            </p>
          </div>

          <div className="journey-track grid md:grid-cols-4 gap-4 mt-16">
            {[
              { num: "01", t: "Documentation", d: "Collate all past medical records, imaging CDs, and valid government ID." },
              { num: "02", t: "Medications", d: "Bring a physical list of all current prescriptions and dietary supplements." },
              { num: "03", t: "Fasting Rules", d: "Adhere to the 12-hour fasting protocol if scheduled for surgery or PET-CT." },
              { num: "04", t: "Arrival Time", d: "Arrive exactly 45 minutes prior to your allocated slot for registration." },
            ].map((step, i) => (
              <div key={i} className="journey-card bg-navy-deep/40 border border-white/5 p-8 hover:bg-navy-deep transition-colors duration-500 group relative overflow-hidden">
                ...
                <p className="text-paper/50 font-light text-sm leading-relaxed relative z-10">{step.d}</p>
        ...
        <div className="grid lg:grid-cols-12 gap-16 lg:gap-24 relative items-start">
          ...
          <div className="hidden lg:block lg:col-span-4 sticky top-40 text-sm">
              ...
              {FAQ_CATEGORIES.map((cat, i) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`font-display text-2xl transition-all duration-500 ${isActive ? "text-magenta italic" : "text-navy-deep group-hover:text-magenta"}`}>
                      {cat.title}
                    </span>
                    {/* Animated underline */}
                    <div className="h-[1px] w-full bg-navy/10 mt-2 relative overflow-hidden">
                      <div className={`absolute inset-y-0 left-0 bg-magenta transition-all duration-700 ease-out ${isActive ? "w-full" : "w-0 group-hover:w-full"}`} />
                    </div>
                  </button>
                );
              })}
            ...
            <div className="mt-20 p-8 bg-surface text-paper shadow-editorial border border-white/10 group cursor-pointer hover:border-magenta/50 transition-colors duration-500">
               <span className="text-[0.55rem] font-syne uppercase tracking-widest text-paper/50 font-bold block mb-4 group-hover:text-magenta">24/7 Support</span>
               <p className="font-display text-3xl leading-tight mb-6">Need direct assistance?</p>
               <a href="/contact" className="inline-flex items-center gap-3 text-[0.65rem] font-syne uppercase tracking-widest text-magenta font-bold">
                 Contact Concierge <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
               </a>
            </div>
          </div>
             ...
             {FAQ_CATEGORIES.map((cat, cIdx) => (
                <div key={cat.id} id={`cat-${cat.id}`} className="scroll-mt-40">
                  <h2 className="cat-heading font-display text-4xl md:text-5xl text-navy-deep mb-12 border-b border-navy/20 pb-8 flex items-end justify-between">
                    <span>{cat.title}</span>
                    <span className="text-[0.6rem] font-syne uppercase tracking-[0.3em] font-normal text-ink/30 block mb-2">SEC 0{cIdx+1}</span>
                  </h2>
      ...
      <section className="relative px-6 md:px-12 lg:px-24 py-32 md:py-48 bg-navy-deep text-paper overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none opacity-20">
            ...
            <img src={hospitalLobby} alt="" className="absolute inset-0 w-full h-full object-cover grayscale mix-blend-overlay opacity-30" />
        </div>
        ...
        <div className="relative z-10 max-w-[1200px] mx-auto text-center" data-reveal>
          ...
          <p className="text-paper/60 font-light text-xl md:text-2xl mb-16 max-w-2xl mx-auto leading-relaxed">
            Our specialized concierge team is available around the clock to provide detailed context to any specific medical, travel, or administrative inquiry.
          </p>
          <div className="flex flex-col sm:flex-row justify-center gap-4">
             <a href="/contact" className="mag-btn group bg-magenta text-paper px-10 py-5 text-[0.65rem] font-syne font-bold uppercase tracking-widest hover:bg-paper hover:text-navy-deep transition-colors duration-500 text-center inline-flex items-center justify-center gap-3">
              Write to us
