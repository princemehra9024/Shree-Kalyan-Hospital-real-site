...
export const Route = createFileRoute("/services")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Explore our specialized medical institutes and centers of excellence." },
    ],
  }),
});
...
function ServicesPage() {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(".hq-word", 
        ...
        { yPercent: 0, skewY: 0, opacity: 1, duration: 1.8, stagger: 0.1, ease: "expo.out", delay: 0.2 }
      );
      gsap.fromTo(".hq-line", 
        { width: 0 }, 
      ...
      if (hoverImage && track) {
        ...
        gsap.utils.toArray<HTMLElement>(".dept-row").forEach((row, i) => {
          row.addEventListener("mouseenter", () => {
            ...
            gsap.to(hoverImage, { scale: 1, opacity: 0.8, rotate: Math.random() * 6 - 3, duration: 0.4, ease: "back.out(1.5)" });
          });
          row.addEventListener("mouseleave", () => {
             gsap.to(hoverImage, { scale: 0.5, opacity: 0, rotate: 0, duration: 0.3, ease: "power2.in" });
          });
        });
  ...
  return (
    <div ref={pageRef} className="bg-paper text-ink font-sans overflow-x-hidden selection:bg-magenta selection:text-white pb-0">
      <SiteNav />
      ...
      <section className="services-hero relative w-full pt-48 md:pt-56 pb-24 px-6 md:px-12 lg:px-24 max-w-[1600px] mx-auto min-h-[90vh] flex flex-col border-b border-navy/10 overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-[60%] md:w-[45%] h-full opacity-10 pointer-events-none mix-blend-multiply overflow-hidden mask-image-b">
          <img src={surgicalTheater} className="hero-parallax-img w-full h-[130%] object-cover grayscale" alt="texture" />
        </div>
        ...
        <div className="mt-auto pt-24 grid md:grid-cols-2 gap-12 items-end relative z-10">
          ...
          <p className="hq-fade text-lg md:text-2xl text-ink/60 font-light leading-relaxed max-w-lg md:text-right md:justify-self-end">
            We treat the unyielding severity of diagnosis with an uncompromising standard of surgical innovation and medical artistry.
          </p>
        </div>
      </section>
        ...
        <div ref={trackRef} className="max-w-[1600px] mx-auto relative relative-track">
          ...
          <img 
            ...
            style={{ 
              ...
              left: 0, top: 0, // rely on GSAP quickTo
            }} 
          />
                ...
                <div className="lg:w-[25%] pointer-events-none">
                  <ul className="grid grid-cols-1 gap-3 border-l border-navy/10 pl-6 lg:pl-10">
                    {dept.services.slice(0, 3).map((s, si) => (
                      <li key={si} className="text-[0.65rem] font-syne font-bold tracking-widest uppercase text-ink/50 flex items-center gap-3">
      ...
      <section className="relative px-6 md:px-12 lg:px-24 py-32 md:py-48 bg-navy-deep text-paper overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
            ...
            <img src={heroCorridor} alt="" className="absolute inset-0 w-full h-full object-cover grayscale mix-blend-overlay opacity-10" />
        </div>

        <div className="max-w-[1200px] mx-auto text-center relative z-10" data-reveal>
          <p className="text-[0.65rem] font-syne font-bold tracking-[0.5em] text-magenta uppercase mb-12">Private Consultation</p>
          <h3 className="font-display text-5xl md:text-7xl lg:text-[6.5rem] leading-[0.9] text-paper mb-16 tracking-tight">
            Seeking a specific <br/> <em className="italic font-light text-magenta/90">medical review?</em>
          </h3>
          <div className="flex flex-col sm:flex-row justify-center gap-6">
             <Link to="/contact" className="group bg-magenta text-paper px-12 py-6 text-[0.65rem] font-syne font-bold tracking-[0.3em] uppercase hover:bg-paper hover:text-navy-deep transition-colors duration-500 inline-flex items-center justify-center gap-4 border border-magenta">
                Contact Admissions Desk
                <ArrowRight className="size-4 group-hover:translate-x-1 transition-transform" />
             </Link>
             <a href="tel:+918529219330" className="bg-transparent border border-white/20 text-paper px-12 py-6 text-[0.65rem] font-syne font-bold tracking-[0.3em] uppercase hover:border-paper transition-colors inline-block text-center mt-4 sm:mt-0">
                Call +91 85292 19330
             </a>
          </div>
        </div>
